...
class Venta
{
  ...
  public function resumend($ano, $mes, $dia, $idempresa)
  {
    $sql = "
select 
idboleta as id,
...
where year(fecha_baja)='$ano' and month(fecha_baja)='$mes' and day(fecha_baja)='$dia' and b.estado='3' and e.idempresa='$idempresa'
union all
select
  idboleta, 
fecha_emision_01 as fecha, 
...
factura f inner join persona p on f.idcliente=p.idpersona  inner join empresa e on f.idempresa=e.idempresa 
where 
f.estado in('1','4','5') and  p.numero_documento='$ndocu' and year(f.fecha_emision_01)='$ano' and month(f.fecha_emision_01) in($mes) 
) as tabla ";
    return ejecutarConsulta($sql);
  }
...
as tabla ";
  ...
  public function regventareporteFacturaDia($idempresa)
  {
    $sql = "select 
        ...
        date(fecha_emision_01)=current_date  and p.tipo_persona='CLIENTE' and f.estado in('5','3','1','6', '4')  and e.idempresa='$idempresa'
             )
             as tabla order by fecha asc";
    return ejecutarConsulta($sql);
  }
  ...
  public function regventareporteBoletaDia($idempresa)
  {
    $sql = "select 
        ...
        hora
            from 
            (select  b.idboleta, b.tipo_documento_06 ,b.fecha_emision_01, b.monto_15_2 as subtotal, b.numeracion_07, b.sumatoria_igv_18_1 as igv, b.importe_total_23 as total, b.estado, p.numero_documento , p.razon_social, date_format(b.fecha_emision_01,'%H  %r') as hora from boleta b inner join persona p on b.idcliente=p.idpersona inner join empresa e on b.idempresa=e.idempresa where date(fecha_emision_01)=current_date and p.tipo_persona='CLIENTE' and b.estado in('5','3', '1','4', '6') and e.idempresa='$idempresa'
            ) as tabla order by fecha asc
             ";
    return ejecutarConsulta($sql);
  }
  ...
  public function ventasxCliente($ndocu, $ano, $mes)
  {
    $sql = "
select 
numero_documento, 
...
descdet  
    from 
    (select
    p.numero_documento, p.razon_social, f.numeracion_08 as numerofac, date_format(f.fecha_emision_01, '%d/%m/%Y') as fechaemision, a.codigo, a.nombre ,format(df.cantidad_item_12,2) as cantidad, df.valor_venta_item_21 as subtotal, df.afectacion_igv_item_16_1 as igv, (df.valor_venta_item_21 + df.afectacion_igv_item_16_1) as total, a.unidad_medida, f.icbper, f.estado, df.descdet
        from
    factura f inner join detalle_fac_art df on f.idfactura=df.idfactura  inner join articulo a on df.idarticulo=a.idarticulo inner join persona p on f.idcliente=p.idpersona inner join empresa e  on f.idempresa=e.idempresa  
    where 
    p.numero_documento='$ndocu'  and year(f.fecha_emision_01)='$ano' and month(f.fecha_emision_01) in($mes)  and f.estado in ('1','4','5')
...
estado
    ...
    where 
    f.estado in('1','4','5','6') and  p.numero_documento='$ndocu' and year(f.fecha_emision_01)='$ano' and month(f.fecha_emision_01) in($mes)  group by f.numeracion_08
    )
    as tabla order by numerofac ";
    return ejecutarConsulta($sql);
  }
...
unidad_medida  
    ...
    union all
    select  p.numero_documento, p.nombres, p.apellidos, b.numeracion_07 as numerobol, date_format(b.fecha_emision_01, '%d/%m/%Y') as fechaemision, a.codigo, a.descripcion as nombre ,format(db.cantidad_item_12,2) as cantidad, db.valor_venta_item_32 as subtotal, db.afectacion_igv_item_monto_27_1 as igv, (db.valor_venta_item_32 + db.afectacion_igv_item_monto_27_1) as total, a.codigo as unidad_medida  
    from 
    boletaservicio b inner join detalle_boleta_producto_ser db on b.idboleta=db.idboleta  inner join servicios_inmuebles a on db.idarticulo=a.id inner join persona p on b.idcliente=p.idpersona inner join empresa e on b.idempresa=e.idempresa where p.numero_documento='$ndocu' and e.idempresa='$idempresa'   
    union all
...
b.importe_total_23 
from
boleta b inner join persona p on b.idcliente=p.idpersona inner join empresa e on b.idempresa=e.idempresa where p.numero_documento='$ndocu' and e.idempresa='$idempresa'
union all
select
b.monto_15_2, 
b.sumatoria_igv_18_1, 
b.importe_total_23 
from
boletaservicio b inner join persona p on b.idcliente=p.idpersona inner join empresa e on b.idempresa=e.idempresa where p.numero_documento='$ndocu' and e.idempresa='$idempresa'
)as tabla";
    return ejecutarConsulta($sql);
  }
...
boletaservicio b inner join persona p on b.idcliente=p.idpersona inner join usuario u on 
  ...
  ///////REPORTE de las VENTAS x DIA MES y AÑO////////

  public function ventasVendedorTotalxMes($vendedor, $ano, $mes, $idempresa)
  {
    $sql = "
SELECT 
    vendedorsitio AS Vendedor,
    SUM(totalFactura) as TotalFactura,
    SUM(totalBoleta) as TotalBoleta,
    SUM(totalNota) as TotalNota
FROM (
    (
        SELECT 
            vendedorsitio,
            SUM(importe_total_venta_27) as totalFactura,
            0 as totalBoleta,
            0 as totalNota 
        FROM factura b 
        INNER JOIN empresa e ON b.idempresa=e.idempresa  
        WHERE vendedorsitio='$vendedor' 
        AND YEAR(fecha_emision_01)='$ano' 
        AND MONTH(fecha_emision_01)='$mes' 
        AND estado IN('5') 
        AND e.idempresa='$idempresa'
        GROUP BY vendedorsitio
    )
    UNION ALL
    (
        SELECT 
            vendedorsitio,
            0 as totalFactura,
            SUM(importe_total_23) as totalBoleta,
            0 as totalNota 
        FROM boleta b 
        INNER JOIN empresa e ON b.idempresa=e.idempresa  
        WHERE vendedorsitio='$vendedor' 
    ...
    (
        SELECT 
            ...
            0 as totalFactura,
            0 as totalBoleta,
            SUM(importe_total_23) as totalNota 
        FROM notapedido b 
        INNER JOIN empresa e ON b.idempresa=e.idempresa  
        WHERE vendedorsitio='$vendedor' 
        AND YEAR(fecha_emision_01)='$ano' 
...
";
  ...
  public function ventasVendedorBoletaxdia($vendedor, $ano, $mes, $dia, $idempresa)
  {
    $sql = "
      SELECT IF(SUM(importe_total_23) IS NULL, '0.00', FORMAT(SUM(importe_total_23), 2)) AS totalBoleta 
      FROM (
          SELECT importe_total_23 
          FROM boleta b 
          INNER JOIN empresa e ON b.idempresa=e.idempresa  
          WHERE b.vendedorsitio='$vendedor' 
          AND YEAR(b.fecha_emision_01)='$ano'
          AND MONTH(b.fecha_emision_01)='$mes'
          AND DAY(b.fecha_emision_01)='$dia'
          AND b.estado IN('5') 
          AND e.idempresa='$idempresa'
          UNION ALL
          SELECT importe_total_23 
  ...
  public function ventasVendedorNotaxdia($vendedor, $ano, $mes, $dia, $idempresa)
  {
    $sql = "
    SELECT IF(SUM(importe_total_23) IS NULL, '0.00', SUM(importe_total_23)) AS totalNota 
    FROM (
        SELECT importe_total_23 
        FROM notapedido b 
        INNER JOIN empresa e ON b.idempresa=e.idempresa  
  ...
  public function ventasVendedorFacturaxdia($vendedor, $ano, $mes, $dia, $idempresa)
  {
    $sql = "
    SELECT IF(SUM(importe_total_venta_27) IS NULL, '0.00', SUM(importe_total_venta_27)) AS totalFactura 
    FROM (
...
where year(nc.fecha_baja)='$ano' and month(nc.fecha_baja)='$mes' and day(nc.fecha_baja)='$dia' and nc.estado='3'
  ...
  public function enviarxmlbajafactura($nombrexml, $idempresa)
  {
    ...
    $client = new SoapClient(
      $webservice,
      ['cache_wsdl' => WSDL_CACHE_NONE, 'trace' => TRUE, 'soap_version' => SOAP_1_1]
    );
  ...
  public function regbajasxmlBoleta($ano, $mes, $dia, $idempresa)
  {
...
where year(fecha_baja)='$ano' and month(fecha_baja)='$mes' and day(fecha_baja)='$dia' and b.estado='3'
  ...
  public function listarcomprobantes($fec1, $fec2, $tipocte, $Estd)
  {
    ...
    $sql = "select 
        ...
        inner join usuario u on f.idusuario=u.idusuario 
        inner join empresa e on f.idempresa=e.idempresa where
        date_format(fecha_emision_01,'%Y-%m-%d') BETWEEN '$fec1' and '$fec2' and f.tipo_documento_07='$tcFactura' and f.estado in ($Estd)
        ...
        inner join usuario u on b.idusuario=u.idusuario 
        inner join empresa e on b.idempresa=e.idempresa where
        date_format(fecha_emision_01,'%Y-%m-%d') BETWEEN '$fec1' and '$fec2' and b.tipo_documento_06='$tcBoleta' and b.estado in ($Estd)
        )
        as estados";
    return ejecutarConsulta($sql);
