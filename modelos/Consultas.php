...
class Consultas
{
  ...
  public function descargarcomprobante($ano, $mes, $dia, $comprobante, $estado, $idempresa) {
    $sql = "SELECT id, tipocomp as tipodocu, date_format(fecha_emision_01, '%Y-%m-%d') as fecha, numer as documento, 
        format(subtotal,2) as subtotal, format(igv,2) as igv, format(total,2) as total  
    from ( select idfactura as id, tipo_documento_07 as tipocomp, fecha_emision_01, numeracion_08 as numer, total_operaciones_gravadas_monto_18_2 as subtotal, 
        sumatoria_igv_22_1 as igv, importe_total_venta_27 as total, f.estado as est
      from factura f 
      ...
      select idboleta as id, tipo_documento_06 as tipocomp, fecha_emision_01, numeracion_07 as numer, monto_15_2 as subtotal, 
        sumatoria_igv_18_1 as igv, importe_total_23 as total, b.estado as est
        from boleta b 
        inner join persona p on b.idcliente=p.idpersona 
        inner join empresa e on b.idempresa=e.idempresa
        where year(fecha_emision_01)='$ano' and month(fecha_emision_01)='$mes' and day(fecha_emision_01)='$dia' and e.idempresa='$idempresa' and tipo_documento_06='$comprobante' and b.estado='$estado'
    ) as tbventa order by fecha";
    return ejecutarConsulta($sql);
  }
  ...
  {
    $sql = "SELECT dia, sum(ventasdia) as ventasdia, max(horaemision) as horaactualizacion
      from (
          select dayofweek(f.fecha_emision_01) as dia, sum(f.importe_total_venta_27) as ventasdia, time(max(f.fecha_emision_01)) as horaemision
            from factura f 
            where yearweek(f.fecha_emision_01, 1) = yearweek(curdate(), 1) 
            group by dayofweek(f.fecha_emision_01)            
            union all             
            select dayofweek(b.fecha_emision_01) as dia, sum(b.importe_total_23) as ventasdia, time(max(b.fecha_emision_01)) as horaemision
            from boleta b 
            where yearweek(b.fecha_emision_01, 1) = yearweek(curdate(), 1) group by dayofweek(b.fecha_emision_01)      
          union all      
          select dayofweek(np.fecha_emision_01) as dia, sum(np.monto_15_2) as ventasdia, time(max(np.fecha_emision_01)) as horaemision
          from notapedido np 
  ...
  public function totalpordianotapedido($ano, $mes)
  {
    ...
    (
      select importe_total_23, day(fecha_emision_01) as dia, 
      dayname(fecha_emision_01) as nombredia 
      from notapedido 
      where month(fecha_emision_01)='$mes' and year(fecha_emision_01)='$ano'
    ) as tabla group by dia";
    return ejecutarConsulta($sql);
  }
  ...
  {
    $sql = "SELECT sum(importe_total_23) as totalboleta , dia, nombredia 
    from (
      SELECT importe_total_23, day(fecha_emision_01) as dia, 
      dayname(fecha_emision_01) as nombredia from boleta where month(fecha_emision_01)='$mes' 
      and year(fecha_emision_01)='$ano' and tipo_moneda_24='$moneda'
...
limit 7;
  ...
  public function insertarArticulosMasivo($codigo, $familia_descripcion, $nombre, $marca, $descrip, $costo_compra, $precio_venta, $stock, $saldo_iniu, $valor_iniu, $tipoitem, $codigott, $desctt, $codigointtt, $nombrett, $nombre_almacen, $unidad_medida, $limitestock)
  {
    $sql = "INSERT INTO articulo( codigo, idfamilia, nombre, marca, descrip, costo_compra, precio_venta, stock, saldo_iniu, valor_iniu, tipoitem, codigott, desctt, codigointtt, nombrett,idalmacen, unidad_medida, umedidacompra, limitestock) 
        VALUES ('$codigo', '$familia_descripcion', '$nombre', '$marca', '$descrip', $costo_compra, $precio_venta, $stock, $saldo_iniu, $valor_iniu, '$tipoitem', '$codigott', '$desctt', '$codigointtt', '$nombrett', '$nombre_almacen', '$unidad_medida', '$unidad_medida', '$limitestock' )";
    return ejecutarConsulta($sql);
  }
  ...
  public function ClientesTop()
  {
    ...
    from 
        persona p
    left join (SELECT idcliente, sum(importe_total_venta_27) as total_factura 
      from factura group by idcliente) f on p.idpersona = f.idcliente
    left join (SELECT idcliente, sum(importe_total_23) as total_boleta 
      from boleta group by idcliente) b on p.idpersona = b.idcliente
    left join (SELECT idcliente, sum(importe_total_23) as total_notapedido 
      from notapedido group by idcliente) n on p.idpersona = n.idcliente
    where p.tipo_documento in ('1', '6') group by p.idpersona, p.razon_social, p.nombres, p.apellidos, p.domicilio_fiscal, p.numero_documento
