...
class Articulo
{
  ...
  public function listar($idempresa)
  {
    ...
    $sql = "SELECT a.idarticulo, f.idfamilia, a.codigo_proveedor, a.codigo, f.descripcion as familia, left(a.nombre, 50) as nombre, 
        ...
        a.ccontable, a.stock as st2, um.nombreum, date_format(a.fechavencimiento, '%d/%m/%Y') as fechavencimiento, al.nombre as nombreal
        from articulo a 
  ...
  public function listarActivosVentaumventa($idempresa, $tpc, $alm, $tipoprecioa)
  {
    switch ($tipoprecioa) {
      case '1':
        ...
        (a.precio_venta) as precio_venta, a.imagen, a.estado, a.unidad_medida, (a.precio_venta * 1.18) as precio_unitario, a.cicbper, a.nticbperi, a.ctticbperi,
        format(a.mticbperu,2) as mticbperu, a.limitestock, um.nombreum, um.abre, um.equivalencia, (a.factorc * a.stock) as factorconversion, a.stock,
        a.factorc, a.descrip, a.tipoitem
        from articulo a 
        inner join familia f on a.idfamilia=f.idfamilia 
        inner join almacen al on a.idalmacen=al.idalmacen 
        inner join empresa e on al.idempresa=e.idempresa 
        inner join umedida um on a.unidad_medida=um.idunidad  
        where a.estado='1' and not a.nombre ='1000ncdg' and e.idempresa='$idempresa' and a.tipoitem='$tpc' and al.idalmacen='$alm' and al.estado='1' order by a.stock desc ";
      break;

      case '2':
        $sql = "SELECT a.idarticulo, a.idalmacen, a.codigo_proveedor, a.codigo, a.nombre, a.idfamilia, f.descripcion as familia, a.costo_compra, (a.precio2) as precio_venta, 
        a.stock, a.imagen, a.estado, a.unidad_medida, (a.precio2 * 1.18) as precio_unitario, a.cicbper, a.nticbperi, a.ctticbperi, format(a.mticbperu,2) as mticbperu,
        a.limitestock, um.nombreum, um.abre, um.equivalencia, (a.factorc * a.stock) as factorconversion, a.stock, a.factorc, a.descrip, a.tipoitem
        from articulo a 
        inner join familia f on a.idfamilia=f.idfamilia 
        inner join almacen al on a.idalmacen=al.idalmacen 
        inner join empresa e on al.idempresa=e.idempresa 
      ...
      case '3':
        $sql = "SELECT a.idarticulo, a.idalmacen, a.codigo_proveedor, a.codigo, a.nombre, a.idfamilia, f.descripcion as familia, a.costo_compra, (a.precio3) as precio_venta, 
        a.stock, a.imagen, a.estado, a.unidad_medida, (a.precio3 * 1.18) as precio_unitario, a.cicbper, a.nticbperi, a.ctticbperi, format(a.mticbperu,2) as mticbperu,
        a.limitestock, um.nombreum, um.abre, um.equivalencia, (a.factorc * a.stock) as factorconversion, a.stock, a.factorc, a.descrip, a.tipoitem
        from articulo a 
        inner join familia f on a.idfamilia=f.idfamilia 
        inner join almacen al on a.idalmacen=al.idalmacen 
...
(select a.codigo, a.nombre, format(sum(db.valor_uni_item_31)/count(db.valor_uni_item_31), 2) as promedio from detalle_boleta_producto db inner join articulo a on db.idarticulo=a.idarticulo inner join boleta b on b.idboleta=db.idboleta where a.idarticulo not in('1') and a.tipoitem not in ('servicios') and year(b.fecha_emision_01)='$ano' and month(b.fecha_emision_01)='$mes' group by codigo
 ...
 as tabla group by codigo";
  ...
  public function totalventasxcodigo($idarticulo, $ano, $mes) {
    $sql = "SELECT sum(tventas) as totalingresoventa 
    from ( 
      SELECT  valor_venta_item_32 as tventas 
      from detalle_boleta_producto db 
      inner join articulo a on db.idarticulo=a.idarticulo
      inner join boleta b on db.idboleta=b.idboleta 
      where db.idarticulo='$idarticulo' and year(b.fecha_emision_01)='$ano' and month(b.fecha_emision_01)='$mes' 
      union all
      ...
      where dn.idarticulo='$idarticulo' and year(np.fecha_emision_01)='$ano' and month(np.fecha_emision_01)='$mes' 
    ) as tabla ";
    return ejecutarConsulta($sql);
  }
