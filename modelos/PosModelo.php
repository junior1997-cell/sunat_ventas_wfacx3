...
class PosModelo
{
  ...
  public function listarProducto($idempresa, $idfamilia = null, $busqueda = null)
  {
    ...
    $sql = "SELECT a.idarticulo, f.idfamilia, a.codigo_proveedor, a.codigo, f.descripcion as familia, left(a.nombre, 50) as nombre, format(a.stock,2) as stock, 
      a.precio_venta as precio, a.costo_compra, (a.precio_venta * 0.18) as precio_unitario, a.cicbper, format(a.mticbperu,2) as mticbperu, a.factorc, a.descrip, a.tipoitem, 
      a.imagen, a.estado, a.precio_final_kardex, a.unidad_medida, a.ccontable, a.stock as st2, um.nombreum, um.abre, date_format(a.fechavencimiento, '%d/%m/%Y') as fechavencimiento, 
      ...
      where not a.nombre='1000ncdg' and e.idempresa='$idempresa' and al.estado='1' $filtro ORDER BY a.nombre ASC";
  ...
  public function listarBoletas($idempresa) {
    ...
    b.tipo_documento_06, b.numeracion_07, format(b.importe_total_23, 2) as importe_total_23, b.estado, p.nombres, p.apellidos, e.numero_ruc, p.email, b.CodigoRptaSunat, 
    b.DetalleSunat, b.tarjetadc, b.montotarjetadc, b.transferencia, b.montotransferencia, b.tipo_moneda_24 as moneda, b.tcambio, (b.tcambio * importe_total_23) as valordolsol,
    ...
    where date(b.fecha_emision_01) = current_date and e.idempresa = '$idempresa' group by b.idboleta order by b.idboleta desc;";
  ...
  public function listarComprobantesVarios($idempresa, $fechainicio, $fechafinal, $tipocomprobante) {
    ...
    if ($tipocomprobante == "Boleta" || $tipocomprobante == "Todos") {
      $sql .= "SELECT  b.idboleta as id,  date_format(b.fecha_emision_01, '%d/%m/%y') as fecha, p.razon_social as cliente, b.estado, 'Boleta' as tipo_comprobante, b.importe_total_23 as total
      from boleta b 
      inner join persona p on b.idcliente = p.idpersona 
    ...
    if ($tipocomprobante == "Factura" || $tipocomprobante == "Todos") {
      // Verificar si ya hay una consulta
      if ($sql != "") { $sql .= " union "; }
      $sql .= "SELECT f.idfactura as id, date_format(f.fecha_emision_01, '%d/%m/%y') as fecha, p.razon_social as cliente, f.estado, 'Factura' as tipo_comprobante, f.importe_total_venta_27 as total
      from factura f 
    ...
    if ($tipocomprobante == "NotaPedido" || $tipocomprobante == "Todos") {
      // Verificar si ya hay una consulta
      if ($sql != "") { $sql .= " union ";  }
      $sql .= "SELECT b.idboleta as id, date_format(b.fecha_emision_01,'%d/%m/%y') as fecha, p.razon_social as cliente, b.estado, 'NotaPedido' as tipo_comprobante,
