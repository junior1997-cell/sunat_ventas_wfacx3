...
switch ($_GET["op"]){
	case 'guardaryeditar':
		...
		if (empty($idplato)){
			$rspta=$plato->insertar($idcategoria,	$codigo, html_entity_decode($nombre , ENT_QUOTES | ENT_HTML401, 'UTF-8'),	$precio, $imagen,	$plato );
			echo $rspta ? "Plato registrado" : "Plato no se pudo registrar, ya existe el código";
		}	else{
			$rspta=$plato->editar($idplato,	$idcategoria,	$codigo, html_entity_decode($nombre , ENT_QUOTES | ENT_HTML401, 'UTF-8'),	$precio, $imagen,	$plato );
	...
	case 'listar':
		...
		$url='../reportes/printbarcode.php?codigopr=';
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
 				"0"=>($reg->estado)?'<a><i class="fa fa-pencil" onclick="mostrar('.$reg->idplato.')" style="color:orange; font-size:22px;" data-toggle="tooltip" title="Editar plato"> </i> </a>'.
