...
switch ($_GET["op"]) {
	case 'guardaryeditar':
		...
		if (empty($idcarga)) {
			$rspta = $ccertificado->insertar(	$numeroruc,	$razon_social,strtoupper($usuarioSol),$claveSol,$rutacertificado,	$rutaserviciosunat,
				$name2 . '.pem',	$keypfx, $webserviceguia
			);
			echo $rspta ? ", Datos almacenados" : "Problemas al guardar";
		} else {
			$rspta = $ccertificado->editar(	$idcarga,	html_entity_decode($numeroruc, ENT_QUOTES | ENT_HTML401, 'UTF-8'), html_entity_decode($razon_social, ENT_QUOTES | ENT_HTML401, 'UTF-8'),
				strtoupper($usuarioSol), $claveSol, $rutacertificado,	$rutaserviciosunat,	$name2 . '.pem', $keypfx, $webserviceguia
			);
