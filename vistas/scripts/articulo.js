...
function listar() {
  ...
  tabla = $('#tbllistado').dataTable({
    ...
    buttons: [
      { text: '<i class="fa-solid fa-arrows-rotate" data-toggle="tooltip" data-original-title="Recargar"></i> ', className: "btn bg-gradient-info m-r-5px", action: function ( e, dt, node, config ) { tabla.ajax.reload(null, false); } },
...
function focuscodprov() {  selectedValue = document.getElementById("tipoitem").value; }
...
function valori(e, field) {
  ...
  if (e.keyCode === 13 && !e.shiftKey) {  document.getElementById('valor_iniu').focus(); }
...
function valorf(e, field) {
  ...
  if (e.keyCode === 13 && !e.shiftKey) { document.getElementById('valor_finu').focus(); }
...
function st(e, field) {
  ...
  // Backspace = 8, Enter = 13, ’0′ = 48, ’9′ = 57, ‘.’ = 46
  key = e.keyCode ? e.keyCode : e.which;
  if (e.keyCode === 13 && !e.shiftKey) { document.getElementById('stock').focus(); }
...
function totalv(e, field) {
  ...
  if (e.keyCode === 13 && !e.shiftKey) { document.getElementById('ventast').focus(); }
...
function porta(e, field) {
  ...
  // Backspace = 8, Enter = 13, ’0′ = 48, ’9′ = 57, ‘.’ = 46
  key = e.keyCode ? e.keyCode : e.which;
  if (e.keyCode === 13 && !e.shiftKey) {  document.getElementById('portador').focus(); }
...
function mer(e, field) {
  ...
  // Backspace = 8, Enter = 13, ’0′ = 48, ’9′ = 57, ‘.’ = 46
  key = e.keyCode ? e.keyCode : e.which
  if (e.keyCode === 13 && !e.shiftKey) { document.getElementById('merma').focus(); }
...
document.getElementById('formularioImportar').addEventListener('submit', function (e) {
  ...
  fetch(this.action, { method: 'POST', body: data }).then(response => response.json()).then(data => {
    if (data.success) {
      Swal.fire({
        icon: 'success',
        title: '¡Éxito!',
