...
function init() {
  ...
  // $(document).ready(function() {  mayor700(); });
...
$("#ipagado").on("input", function () {
  ...
  // Removemos las comas del total para convertirlo correctamente a número
  var totalString = $("#total").text().replace(/,/g, '');
  var total_pagar = parseFloat(totalString);
...
function agregarClientexDoc(e) {
  var dni = $("#numero_documento").val();
  $(".buscando_cliente").html(`<i class="fas fa-spinner fa-pulse fa-lg text-danger"></i>`);
  if (e.keyCode === 13 && !e.shiftKey) {
    $("#razon_social").val("");
    $("#domicilio_fiscal").val("");
