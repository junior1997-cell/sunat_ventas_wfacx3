...
if (!isset($_SESSION["nombre"])) {
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["dataArticulo"])) {
    ...
    // $spreadsheet = new IOFactory($tmpName);
    ...
    for ($i = 2; $i <= $numerofila; $i++) {
      ...
      if ($codigo != "") {
        ...
        $unidad_medida = $excel->getActiveSheet()->getCell('Q' . $i)->getCalculatedValue();
        $limitestock = $excel->getActiveSheet()->getCell('R' . $i)->getCalculatedValue();
